...
PlotDynamicDistribution <-
  function(curves,
    ...
    ##     and columns represent ordinates of x.  I.e. a long time
    ##     series would be a wide matrix.
    ##   x: An optional vector of ordinates that 'curves' will be
    ##     plotted against.  The length of 'x' must match the number
    ##     of columns in 'curves'.
    ...
    .FilledPlot <- function(x,
      ...
      stopifnot(length(x) == nrow(quantile.matrix))
      ...
      if (!add) {
        plot(x, ylo, ylim = ylim, xlab = xlab, ylab = ylab, type = "n", ...)
      }
    ...
    box()
    if (inherits(x, "Date")) {
      axis.Date(1, x, xpd = NA)
    } else if (inherits(x, "POSIXt")) {
      axis.POSIXct(1, as.POSIXct(x), xpd = NA)
    } else {
      axis(1, xpd = NA)
    }
    axis(2, xpd = NA)
