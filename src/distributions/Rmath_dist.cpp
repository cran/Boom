...
#undef qnorm
  ...
  double qgamma(double x, double a, double b, bool low, bool log){
    return Rmath::qgamma(x,a,1.0/b,low,log);}
  double rgamma(double a, double b){
    return Rmath::rgamma(a, 1.0/b);}
  ...
  double qlnorm(double x, double mu, double sig, bool low, bool log){
    return Rmath::qlnorm(x,mu,sig,low,log);}
  double rlnorm(double mu, double sig){
    return Rmath::rlnorm(mu,sig);}
  ...
  void rmultinom_mt(BOOM::RNG & rng,
                    ...
                    std::vector<int> &rN){
    Rmath::rmultinom_mt(rng, n, prob, rN);
  }
  ...
  double rlogis(double mu, double sig){
    return Rmath::rlogis(mu,sig);}
