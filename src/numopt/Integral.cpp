...
// Contents of this file were cribbed from R source (which originally
// cribbed them from netlib's public domain quadpack).  Modifed by
// Steven Scott to work with modern C++.

// Original R code used as much as possible.  The main changes were:
  ...
  void operator()(double *x, int n, void*){
    for (int i = 0; i < n; ++i) {
      ...
      if(!std::isfinite(x[i])){
        if(throw_on_error_) {
          BOOM::report_error(
              "Infinite function value in numerical integration");
        }
      }
    }
  }
...
inline int imax2(int a, int b){return std::max<int>(a,b);}
inline double fmin2(double a, double b){return std::min<double>(a,b);}
inline int imin2(int a, int b){return std::min<int>(a,b);}
...
void Rdqagi(integr_fn f,  // integrand
            void *ex,     // NULL pointer
            double *bound, // finite bound of original integration range (or 0)
            int *inf,      // -1 means (-infinity, bound),
                           // 1 means (bound, infinity),
                           // 2 means -inf..inf
	    double *epsabs, // absolute accuracy requested
            double *epsrel, // relative accuracy requested
	    double *result, // output:  value of the integral
            double *abserr, // estimate of the modulus of the absolute error,
            int *neval,     // number of function evaluations
            int *ier,       // error code (see below).  0 means okay
	    int *limit,     // dimension of iwork
            int *lenw,      // dimension of work
            int *last,      // number of partitions produced
	    int *iwork,     // workspace
...
void Rdqags(integr_fn f, // integrand
            void *ex,    // NULL pointer
	    ...
	    double *result,  // output
            double *abserr,  // output:  estimate of the modulus of the  absolute error,
            int *neval,      // output: number of integrand evaluations
            int *ier,        // output: 0 means anwer is reliable. > 0 is an error.  see below
	    int *limit,      // dimension of iwork
            int *lenw,       // dimension of work, must be >= limit*4
            int *last,       // number of subintervals produced
            int *iwork,      // workspace, see below for contents
            double *work     // workspace, see below for contents
            )
{
    int l1, l2, l3;
...
namespace BOOM{
  ...
  Integral::Integral(const Fun &integrand,
                     double lo,
                     double hi,
                     int limit)
      : f_(integrand),
        ...
        work_(4 * limit_),
        iwork_(limit_),
        rel_tol_(pow(std::numeric_limits<double>::epsilon(), .25)),
        abs_tol_(rel_tol_),
        result_(0),
        abs_err_(0),
        neval_(0),
  ...
  void Integral::set_absolute_epsilon(double eps){
    abs_tol_ = eps;
  }

  void Integral::set_relative_epsilon(double eps){
    rel_tol_ = eps;
  }

  void Integral::throw_on_error(bool tf){
    throw_on_error_ = tf;}
  ...
  double Integral::absolute_error()const{return abs_err_;}
  int Integral::number_of_function_evaluations()const{
    return neval_;}
  int Integral::number_of_partitions()const{
    return npartitions_;}
  ...
  std::string Integral::error_message()const{
    ...
    if(error_code_ == 0){
      ...
      msg << "maximum number of subdivisions allowed "
          << "has been achieved. one can allow more "
          << "subdivisions by increasing the value of "
          << "limit (and taking the according dimension "
          << "adjustments into account). however, if "
      ...
      msg << "extremely bad integrand behaviour occurs at "
          << "some points of the integration";
    }else if(error_code_ == 4){
      msg << "The algorithm does not converge.  "
      ...
      msg << "The integral is probably divergent, or slowly convergent.  "
          << "It must be noted that divergence can occur with "
          << "any other value of ier.";
    }else if(error_code_==6){
      msg << "The input is invalid, because (epsabs <= 0 and "
          << "epsrel < max(50*rel.mach.acc.,0.5d-28)) "
          << "or limit < 1 or leniw < limit*4. result, abserr, neval, "
          << "last are set to zero. exept when limit or leniw is "
          << "invalid, iwork(1), work(limit*2+1) and work(limit*3+1) "
          << "are set to zero, work(1) is set to a and work(limit+1) to b.";
    }
    return msg.str();
  }
