...
namespace BOOM{
  ...
  {
    set(MonthNames(m), dd, yyyy);
  }

  Date::Date(MonthNames m, int dd, int yyyy)
  {
    set(m, dd, yyyy);
  }
  ...
  Date & Date::set(const tm &rhs) {
    return set(MonthNames(rhs.tm_mon + 1),
               rhs.tm_mday,
               rhs.tm_year + 1900);
  }
  ...
  Date & Date::set(int days_after_jan_1_1970){
    ...
    int days_into_year;
    year_ = 1970 + years_after_jan_1_1970(days_after_jan_1_1970, &days_into_year);
  ...
  void Date::find_month_and_day(int days_after_jan1, bool leap,
    ...
    const int *begin = leap ?
        days_before_month_in_leap_year_ :
        days_before_month_;
    ++begin;
  ...
  int Date::years_before_jan_1_1970(int days_before, int *days_remaining){
    ...
    *days_remaining = days_before - 365 * (lower_bound)
        - number_of_leap_years_before_1970(year);
  ...
  int Date::years_after_jan_1_1970(int days, int *days_remaining){
    ...
    *days_remaining = days
        - (year - 1970) * 365
        - number_of_leap_years_after_1970(year);
    while(*days_remaining >= 365 + is_leap_year(year)){
      ++year;
      *days_remaining = days
          - (year - 1970) * 365
  ...
  int Date::days_before_jan_1_1970(MonthNames month, int day, int year){
    ...
    int days_to_jan1 =
        366 + leap - days_into_year(month, day, leap);
    ++year;
  ...
  Date & Date::operator=(const struct tm &rhs){
    return set(rhs);
  }
  ...
  int Date::days_until(DayNames day)const{
    ...
    else return 7 - int(today - day);
  }

  int Date::days_after(DayNames day)const{
    DayNames today = day_of_week();
    if(today >= day) return today - day;
    else return 7 + today - day;
  }

  long Date::days_after_jan_1_1970()const{
    return days_after_origin_;
  }
  ...
  Date & Date::operator+=(int n){
    ...
    if(n < days_left_in_month()) day_ += n;
    else set(days_after_origin_);
    return *this;
  }
  ...
  std::ostream & Date::display_month(std::ostream &out)const{
    ...
    static const char * month_abbrevs[]=
      {"", "jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep",
       "oct", "nov", "dec"};
    if(month_format==Full) out << Month_names[month()];
    else if(month_format==full) out << month_names[month()];
    else if(month_format==Abbreviations) out << Month_abbrevs[month()];
    else if(month_format==abbreviations) out << month_abbrevs[month()];
    else out << int(month());
    return out;
  }
  ...
  Date last_weekday_in_month(DayNames day, MonthNames month, int year){
    Date ans(month,
             Date::days_in_month(month, Date::is_leap_year(year)),
             year);
