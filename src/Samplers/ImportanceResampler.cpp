...
namespace BOOM {
  ...
  std::pair<Matrix, Vector> ImportanceResampler::draw(
      int number_of_draws, RNG &rng) {
    ...
    for (int i = 0; i < number_of_draws; ++i) {
      ...
      importance_weights[i] = log_target_density_(proposed_draw)
          - proposal_->logp(proposed_draw);
      max_log_importance_weight = std::max<double>(
          max_log_importance_weight, importance_weights[i]);
    }
    importance_weights -= max_log_importance_weight;
    importance_weights.normalize_logprob();

    std::vector<int> resampling_counts = rmultinom_mt(
        rng, number_of_draws, importance_weights);
    ...
    Matrix unique_draws(number_of_distinct_draws,
                        proposal_draws[0].size());
