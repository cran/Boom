...
namespace BOOM{
  namespace IRT{
    ...
    bool PBC::check(const Vec &b)const{
      uint b_sz = b.size();
    ...
    Vec PBC::reduce(const Vec &b)const{
      uint b_sz = b.size();
      assert(b_sz>=2);
      Vec ans(b_sz-1);
    ...
    Vec PBC::expand(const Vec &b)const{
      Vec ans(b.size()+1);
    ...
    bool PDC::check(const Vec &d)const{
      return(d[0]==0.0 && d.sum()==0.0);}

    void PDC::impose(Vec &d)const{
      double d0 = d[0];
      d-= d0;
      d.back()-= d.sum();
    }

    Vec PDC::reduce(const Vec &d_full)const{
      if(d_full.size()<=2) return Vec(0);
      Vec ans(d_full.size()-2);
      std::copy(d_full.begin()+1, d_full.end()-1, ans.begin());
      return ans;
    }

    Vec PDC::expand(const Vec &d_min)const{
      Vec ans(d_min.size()+2);
      ans[0] = 0.0;
    ...
    Ptr<CVP> make_d_vec(const Vec &d, bool id_d0);

    Ptr<CVP> make_d_uint(uint Maxscore, bool id_d0){
      Vec d(Maxscore+1, 0.0);
    ...
    const Vec & PCR::d()const{ return D_prm()->value(); }

    void PCR::set_a(double A){ A_prm()->set(A); a_current=true;}
    void PCR::set_b(double B){ B_prm()->set(B); b_current=true;}
    void PCR::set_d(const Vec &D){
    ...
    const Vec & PCR::beta()const{ return Beta_prm()->value();}

    void PCR::set_beta(const Vec &b){
      beta_->set(b);
      beta_current=true;
    }

    const Vec & PCR::fill_eta(const Vec & Theta)const{
      eta_ = X(Theta)*beta();
      return eta_;
    }

    const Mat & PCR::X(const Vec &Theta)const{
      return X(Theta[which_subscale()]); }

    const Mat & PCR::X(double theta)const{
      VectorView v(X_.last_col());
      v[0] = theta;
    ...
    double PCR::response_prob(Response r, const Vec &Theta, bool logsc)const{
      return response_prob(r->value(), Theta, logsc);}

    double PCR::response_prob(uint r, const Vec & Theta, bool logsc)const{
      fill_eta(Theta);
    ...
    void PCR::impose_beta_constraint(){
      b_ = beta();
    ...
    void PCR::fill_beta(bool first_time)const{
      ...
      const Vec &d(first_time ? D_prm(false)->value() : this->d());
