...
namespace BOOM{
  ...
  void Ar1Suf::Update(const DoubleData &d){
    update_raw(d.value()); }
  ...
  void Ar1Suf::combine(Ptr<Ar1Suf> rhs){
    this->combine(*rhs);
  }
  ...
  double Ar1Suf::model_sumsq(double mu, double phi)const{
    ...
    ss += sumsq_excluding_first()
        - 2*phi*cross_
        - 2*(1-phi)*mu*sum_excluding_first()
        + phi*phi*lag_sumsq()
  ...
  double Ar1Suf::centered_cross(double mu)const{
    return cross_ - mu * (sum_excluding_first() + lag_sum()) + (n_-1)*mu*mu;
  }
  ...
  double NonzeroMeanAr1Model::pdf(Ptr<Data> dp, bool logscale)const{
  ...
  double NonzeroMeanAr1Model::sigma()const{return sqrt(Sigsq_prm()->value());}
  ...
  const Ptr<UnivParams> NonzeroMeanAr1Model::Sigsq_prm()const{return prm3();}
}
