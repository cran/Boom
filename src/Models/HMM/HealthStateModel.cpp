...
namespace BOOM{
  ...
  HealthStateData::HealthStateData(Ptr<Data> data, int treatment)
      : value_(data),
  ...
  uint HealthStateModel::state_space_size()const{
    return mix_.size();}

  uint HealthStateModel::ntreatments()const{
    return mark_.size();
  }
  ...
  int HealthStateModel::sample_treatment(Ptr<HealthStateData> data,
    ...
    double post_first = prior_first * mark_[first_treatment]->Q()(r,s);
  ...
  void HealthStateModel::fill_logp(Ptr<HealthStateData> dp){
    fill_logp(dp, logp_); }
  ...
  void HealthStateModel::fill_logQ(Ptr<HealthStateData> dp, Matrix &logQ)const{
    ...
    if(alpha >= 1.0){
      ...
      logQ = log((1-alpha) * mark_[then]->Q() +
                 alpha * mark_[now]->Q());
    }
  }

  double HealthStateModel::initialize_fwd(Ptr<HealthStateData> d)const{
    fill_logp(d, logp_);
