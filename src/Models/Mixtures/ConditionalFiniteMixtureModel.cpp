...
namespace BOOM {
  ...
  const Data * ConditionalMixtureData::data() const {
    return data_.get();
  }

  Ptr<Data> ConditionalMixtureData::shared_data() {
    return data_;
  }
  ...
  void ConditionalMixtureData::set_mixture_component(int mixture_indicator) {
    if (known_mixture_component_ > 0
      ...
      err << "A data point knownn to come from mixture component "
          << known_mixture_component_
          << " is being set to mixture component "
          << mixture_indicator
          << ".";
        ...
        mixing_distribution_(mixing_distribution)
  {
    if (mixing_distribution_->Nchoices() != mixture_components.size()) {
      ...
      err << "The number of mixture components: " << mixture_components.size()
          ...
          << " in ConditionalFiniteMixtureModel constructor."
          << endl;
      report_error(err.str());
    }
  }
  ...
  void CFMM::add_data(Ptr<Data> abstract_data_point) {
    add_conditional_mixture_data(
        abstract_data_point.dcast<ConditionalMixtureData>());
  }

  void CFMM::add_conditional_mixture_data(
      Ptr<ConditionalMixtureData> data_point) {
    data_.push_back(data_point);
    mixing_distribution_->add_data(
        data_point->shared_mixture_category_data());
  }

  std::vector<Ptr<ConditionalMixtureData> > & CFMM::dat() {
    return data_;
  }
  ...
  void ConditionalFiniteMixtureModel::impute_latent_data(RNG &rng) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (data_point.missing()) {
        ...
        last_loglike_ +=
            mixture_component(source)->pdf(
                data_point.data(), true);
        ...
        for (int s = 0; s < S; ++s) {
          wsp_[s] =
              mixing_distribution_->predict_subject(
                  mixture_category_data,
                  s)
              + mixture_component(s)->pdf(
                  data_point.data(),
                  true);
        }
        ...
        mixture_component(mixture_indicator)->add_data(
            data_point.shared_data());
      }
    }
  }
  ...
  void CFMM::set_mixture_component_for_observation(
      int i, int mixture_component) {
