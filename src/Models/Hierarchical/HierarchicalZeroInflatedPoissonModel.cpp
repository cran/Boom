...
namespace BOOM {
  ...
  ZeroInflatedPoissonData::ZeroInflatedPoissonData(
      const ZeroInflatedPoissonSuf &suf) : suf_(suf)
  {}
  ...
  HZIP::HierarchicalZeroInflatedPoissonModel(
      ...
      : prior_for_lambda_(new GammaModel(
            lambda_prior_guess * lambda_prior_sample_size,
  ...
  HZIP::HierarchicalZeroInflatedPoissonModel(
      Ptr<GammaModel> prior_for_lambda,
      Ptr<BetaModel> prior_for_zero_probability)
        ...
        prior_for_zero_probability_(new BetaModel(1.0, 1.0))
  {
    ...
    if (trials.size() != events.size() ||
      ...
      report_error("The trials, events, and number_of_zeros arguments must all "
    ...
    for (int i = 0; i < ngroups; ++i) {
      ...
      model->set_sufficient_statistics(
          ZeroInflatedPoissonSuf(number_of_zeros[i],
                                 trials[i] - number_of_zeros[i],
                                 events[i]));
  ...
  void HZIP::add_data_level_model(
      Ptr<ZeroInflatedPoissonModel> model) {
  ...
  void HZIP::add_data(Ptr<Data> dp) {
  ...
  int HZIP::number_of_groups() const {
  ...
  GammaModel * HZIP::prior_for_poisson_mean() {
    return prior_for_lambda_.get();
  }
