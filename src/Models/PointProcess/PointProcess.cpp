...
namespace BOOM{
  ...
  {}
   ...
   PointProcessEvent::PointProcessEvent(const DateTime &time, Ptr<Data> mark)
       : timestamp_(time),
         mark_(mark)
   {}
  ...
  PointProcess::PointProcess(const DateTime &begin,
                             const DateTime &end)
      : begin_(begin),
        end_(end),
  ...
  void PointProcess::check_legal_event_number(int i)const{
    if(events_.empty() || i < 0 || i >= events_.size()){
      ...
      err << "An illegal event number " << i
          << " was passed to a PointProcess containing "
          << events_.size() << " events." << endl;
      report_error(err.str());
    }
  }
  ...
  PointProcess * PointProcess::clone()const{
    return new PointProcess(*this);}
  ...
  uint PointProcess::number_of_events()const{
    return events_.size();
  }

  double PointProcess::window_duration()const{
    return end_ - begin_;
  }
  ...
  PointProcess & PointProcess::append(const PointProcess &rhs){
    ...
    if(immediately_follows(rhs)){
      ...
      err << "You can only append point processes that adjoin "
          ...
          << "The process you're trying to append covers from "
          << rhs.begin_ << " to " << rhs.end_
          << endl;
      report_error(err.str());
    }
    return *this;
  ...
  PointProcess & PointProcess::add_event(const DateTime &event, Ptr<Data> mark){
    PointProcessEvent e(event, mark);
    return add_event(e);
  }

  const DateTime & PointProcess::window_begin()const{
    return begin_;
  }

  const DateTime & PointProcess::window_end()const{
    return end_;
  }
