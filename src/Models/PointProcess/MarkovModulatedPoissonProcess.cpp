...
namespace BOOM{
  ...
  namespace {
    ...
    dumb(const std::vector<Ptr<PoissonProcess> > &arg){
  ...
  namespace MmppHelper {
    ...
    // class.
    HmmState::HmmState(std::vector<PoissonProcess *> processes)
        : active_processes_(processes),
          id_number_(-1)
    {
      if(processes.empty()){
    ...
    const std::vector<HmmState *> &
        HmmState::potential_outgoing_transitions()const{
      return potential_outgoing_transitions_;
    }

    const std::vector<HmmState *> &
        HmmState::potential_incoming_transitions()const{
    ...
    void HmmState::add_transition_from(HmmState *previous_state){
      std::vector<HmmState *>::iterator it = std::lower_bound(
          ...
          potential_incoming_transitions_.end(),
          previous_state);
    ...
    void HmmState::add_transition_to(HmmState *next_state,
      ...
      std::vector<HmmState *>::iterator it = std::lower_bound(
          potential_outgoing_transitions_.begin(),
          potential_outgoing_transitions_.end(),
          next_state);
      if(it == potential_outgoing_transitions_.end()
          || *it != next_state){
          ...
          processes_(processes)
        {
          ...
          if(!mixture_components.empty()){
            if(mixture_components.size() != processes.size()){
              report_error("Arguments are of different sizes in "
            ...
            std::unique_copy(mixture_storage.begin(),
                             mixture_storage.end(),
            ...
            std::map<PoissonProcess *, MixtureComponent *>
                mixture_component_map;
            ...
            for(int i = 0; i < processes_.size(); ++i){
              ...
              std::vector<MixtureComponent *>::iterator it = std::lower_bound(
                  minimal_mixture_components_.begin(),
                  minimal_mixture_components_.end(),
                  mix);
              if(*it != mix){
                report_error("Error finding mixture components in "
                             "ProcessInfo constructor.");
              }
    ...
    void ProcessInfo::evaluate(const PointProcess &data,
      ...
      for(int t = 0; t < data.number_of_events(); ++t){
        const DateTime &t0(
            t == 0 ? data.window_begin() : data.event(t-1).timestamp());
        ...
        if(data.event(t).has_mark() && !(minimal_mixture_components_.empty())){
    ...
    double ProcessInfo::log_event_rate(
        const PoissonProcess *process, int t) const {
    ...
    // returned.
    double ProcessInfo::mixture_log_likelihood(
        const PoissonProcess *process, int t) const {
      if(minimal_mixture_components_.empty()){
        return 0.0;
      }
    ...
    // 'evaluate'.
    double ProcessInfo::conditional_cumulative_hazard(
        const HmmState *state, int t)const{
      double ans = 0;
  ...
  void MMPP::add_component_process(
      Ptr<PoissonProcess> process,
  ...
  // is added.
  void MMPP::add_data(Ptr<Data> dp){
    Ptr<PointProcess> d = DAT(dp);
    add_data(d);
  }
  void MMPP::add_data(Ptr<PointProcess> dp){
  ...
  void MMPP::add_supervised_data(Ptr<PointProcess> dp,
                                 const SourceVector &source){
    add_data(dp);
  ...
  double MMPP::fwd_1(int t,
    ...
    for(int r = 0; r < S; ++r){
      ...
      for(StateVector::const_iterator it = potential_states.begin();
        ...
        P(r, s) = log_prior_hazard + conditional_event_loglikelihood(
  ...
  void MMPP::backward_sampling(RNG &rng,
  ...
  //----------------------------------------------------------------------
  int MMPP::hmm_state_space_size()const{
    return hmm_states_.size();
  }

  //----------------------------------------------------------------------
  int MMPP::number_of_processes()const{
    return component_processes_.size();
  }
  ...
  double MMPP::conditional_event_loglikelihood(
    ...
    if(nproc == 1){
      ...
      report_error("potential_culprits was empty in "
  ...
  // 'previous_state'.
  void MMPP::update_exposure_time(
      const PointProcess &process, int t, int previous_state){
    const DateTime &then(
        t > 0 ?
        process.event(t-1).timestamp()
        : process.window_begin());
    const DateTime &now(
        t < process.number_of_events() ?
  ...
  void MMPP::check_that_all_processes_have_been_registered(){
    for(int i = 0; i < component_processes_.size(); ++i){
      ...
      if(spawns_.find(process) == spawns_.end()){
        report_error("At least one process listed in 'spawns' or "
  ...
  // add_component_process().
  void MMPP::check_first_entry(Ptr<PoissonProcess> process_ptr){
    PoissonProcess *process = process_ptr.get();
    if(spawns_.find(process) != spawns_.end()){
      report_error("A process was submitted twice to "
  ...
  void MMPP::check_for_new_process(Ptr<PoissonProcess> process){
    for(int i = 0; i < component_processes_.size(); ++i){
  ...
  void MMPP::check_for_new_mixture_component(Ptr<MixtureComponent> component){
    if(!component){
      return;
    }
    for(int i = 0; i < mixture_components_.size(); ++i){
  ...
  // state.
  void MMPP::generate_new_states(Ptr<HmmState> state){
