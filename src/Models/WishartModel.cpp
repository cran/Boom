...
namespace BOOM{
  ...
  const Ptr<SpdParams> WM::Sumsq_prm() const {return ParamPolicy::prm2();}

  const double & WM::nu() const {return ParamPolicy::prm1_ref().value();}
  const SpdMatrix &WM::sumsq() const {return ParamPolicy::prm2_ref().value();}
  ...
  double WishartModel::dloglike(
      const Vector &sumsq_triangle_nu, Vector &g) const {
