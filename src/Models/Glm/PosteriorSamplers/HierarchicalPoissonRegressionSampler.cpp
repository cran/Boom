...
namespace BOOM {
  ...
  {
    if (!model) {
      report_error("NULL model passed to "
                   "HierarchicalPoissonRegressionPosteriorSampler "
                   "constructor.");
    }
    if (model->xdim() != mu_prior->dim()) {
      report_error("The model and mu_prior arguments do not conform in "
                   "HierarchicalPoissonRegressionPosteriorSampler "
                   "constructor.");
    }
  }
            ...
            upper_sigma_truncation_point))
  {
    if (model->xdim() != siginv_priors.size()) {
      report_error("model and 'siginv_priors' arguments do not conform in "
                   "HierarchicalPoissonRegressionIndependencePosteriorSampler "
                   "constructor.");
    }
    if (model->xdim() != upper_sigma_truncation_point.size()) {
      report_error("model snad upper_sigma_truncation_point arguments do not "
                   "conform in "
                   "HierarchicalPoissonRegressionIndependencePosteriorSampler "
                   "constructor.");
    }
