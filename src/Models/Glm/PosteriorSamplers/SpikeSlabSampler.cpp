...
namespace BOOM {
  ...
  SSS::SpikeSlabSampler(GlmModel *model,
  ...
  void SSS::draw_model_indicators(RNG &rng,
    ...
    std::vector<int> indx = seq<int>(
        0,
        inclusion_indicators.nvars_possible()-1);
    ...
    if(!std::isfinite(logp)){
      ...
      err << "SpikeSlabSampler did not start with a "
          << "legal configuration."
          << endl << "Selector vector:  " << inclusion_indicators << endl
  ...
  void SSS::draw_beta(RNG &rng, const WeightedRegSuf &suf, double sigsq) {
    ...
    SpdMatrix precision = inclusion_indicators.select(
        slab_prior_->siginv());
  ...
  double SSS::logpri() const {
    ...
    if(inclusion_indicators.nvars() > 0){
      ans += dmvn(model_->included_coefficients(),
                  inclusion_indicators.select(slab_prior_->mu()),
                  inclusion_indicators.select(slab_prior_->siginv()),
                  true);
    }
    return ans;
  }

  void SSS::allow_model_selection(bool tf) {
    allow_model_selection_ = tf;
  }

  void SSS::limit_model_selection(int max_flips) {
