...
// 			      const Matrix &Xsubject_info);
  ...
  Vector & MNP::eta(Ptr<ChoiceData> dp, Vector & ans)const{
    const Mat&X(dp->X());
  ...
  void MNP::impute_u(RNG &rng, Vector &u, Ptr<ChoiceData> dp, TrunMvnTF & target){
    ...
    else if(imp_method==Gibbs) impute_u_Gibbs(rng, u, dp, target);
    else report_error("unrecognized method in impute_u");
  }

  void MNP::impute_u_slice(Vector &u, Ptr<ChoiceData> dp, TrunMvnTF & target){
    // slice sampler
    eta(dp, wsp);
    target.set_mu(wsp);
    uint y = dp->value();
  ...
  void MNP::impute_u_Gibbs(RNG &rng, Vector &u, Ptr<ChoiceData> dp, TrunMvnTF & ){
    uint y = dp->value();
    wsp = u;
