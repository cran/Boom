...
VECTOR_OUT & matrix_multiply_impl(const MATRIX &m, const VECTOR_IN &v,
  ...
  Vector Matrix::real_evals()const{
    ...
    tmp.reserve(n);
    for(int i=0; i<n; ++i){
      if(i==n-1) tmp.push_back(real_ev[i]);
      else if(real_ev[i]==real_ev[i+1] && imag_ev[i] == -imag_ev[i+1]) ++i;
      else tmp.push_back(real_ev[i]);
    }
